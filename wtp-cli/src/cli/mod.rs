...
pub async fn run() -> anyhow::Result<()> {
    ...
    // Load global config
    let (loaded_config, warning) = wtp_core::LoadedConfig::load()?;
    ...
    if let Err(_) = wtp_core::fence::init_global_fence(loaded_config.config.workspace_root.clone())
    {
        eprintln!("{}", "Warning: Security fence already initialized".yellow());
    }

    // Execute command
    match cli.command {
        Commands::Cd(args) => {
            cd::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Create(args) => {
            create::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Ls(args) => {
            ls::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Remove(args) => {
            remove::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Status(args) => {
            status::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Eject(args) => {
            eject::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        ...
        Commands::Host(args) => {
            host::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
        Commands::Config(args) => {
            config::execute(args, wtp_core::WorkspaceManager::new(loaded_config)).await?
        }
