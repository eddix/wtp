...
async fn add_host(
    ...
    if !path_buf.exists() {
        ...
        eprintln!(
            "The host will be added, but repositories under it won't be accessible until the directory is created."
        );
    }
    ...
    // Add host to config
    manager.global_config_mut().hosts.insert(
        alias.clone(),
        wtp_core::config::HostConfig { root: path_buf },
    );
...
async fn list_hosts(manager: WorkspaceManager) -> anyhow::Result<()> {
    ...
    if default.is_none() {
        ...
        println!(
            "{}",
            "No default host set. Use 'wtp host set-default <alias>' to set one.".dimmed()
        );
    }
...
async fn remove_host(alias: String, mut manager: WorkspaceManager) -> anyhow::Result<()> {
    ...
    // Remove host
    manager.global_config_mut().hosts.shift_remove(&alias);
...
async fn set_default_host(alias: String, mut manager: WorkspaceManager) -> anyhow::Result<()> {
