...
/// Reject values that look like git options.
...
impl GitClient {
        ...
        if is_bare {
            ...
            if let Some(dir) = cwd {
                return Ok(dir
                    .join(git_path)
                    .canonicalize()
                    .map_err(|_| WtpError::NotInGitRepo)?);
            }
        }
    ...
    pub fn get_head_commit(&self, repo_path: &Path) -> Result<String> {
        self.run_git(repo_path, &["rev-parse", "--short", "HEAD"])
    }

    /// Combined HEAD info: (short_hash, subject, relative_time)
    /// Fetches all three values in a single `git log` invocation.
    ...
    pub fn add_worktree_for_branch(
        ...
        if !output.status.success() {
            ...
            return Err(WtpError::git(format!("Failed to add worktree: {}", stderr)));
        }
    ...
    pub fn get_last_commit_subject(&self, repo_path: &Path) -> Result<String> {
        self.run_git(repo_path, &["log", "-1", "--format=%s"])
    }
    ...
    pub fn get_last_commit_relative_time(&self, repo_path: &Path) -> Result<String> {
        self.run_git(repo_path, &["log", "-1", "--format=%cr"])
    }
    ...
    pub fn remove_worktree(
        ...
        let mut cmd = Command::new("git");
        cmd.current_dir(repo_path).arg("worktree").arg("remove");
        if force {
            cmd.arg("--force");
        }
    ...
    /// base not in HEAD. Returns `Ok(None)` if the base ref does not exist
...
/// Combined git status + stash info from a single `git status` invocation.
#[derive(Debug, Clone, Default)]
pub struct FullGitStatus {
    /// Regular status info
