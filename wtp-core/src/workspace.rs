...
use crate::config::{GlobalConfig, LoadedConfig, WTP_DIR};
...
impl WorkspaceManager {
    ...
    async fn run_create_hook(&self, name: &str, path: &Path) -> Result<Option<String>> {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| WtpError::config(format!("Failed to execute create hook: {}", e)))?;
    ...
    fn initialize_workspace_dir(&self, path: &Path, fence: &Fence) -> Result<()> {
        ...
        let worktree_toml = crate::worktree::WorktreeToml::new();
    ...
    pub fn remove_workspace(&mut self, name: &str, delete_dir: bool) -> Result<Option<PathBuf>> {
        ...
        if let Some(ref p) = path {
            if delete_dir && p.exists() {
                crate::fence::ensure_fence(&self.loaded_config.config).remove_dir_all(p)?;
            }
        }
    ...
    pub fn get_hosts(&self) -> &IndexMap<String, crate::config::HostConfig> {
    ...
    pub fn detect_workspace_from(&self, start: &Path) -> Result<(String, PathBuf)> {
        ...
        loop {
            if check_dir.join(WTP_DIR).is_dir() {
                ...
                // Fallback: use dir name directly (no full scan needed)
                let name = check_dir
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("workspace")
